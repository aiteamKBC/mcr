...
var IsRestoringContext = React13.createContext(false);
var useIsRestoring = () => React13.useContext(IsRestoringContext);
...
var QueryErrorResetBoundaryContext = React14.createContext(createValue());
var useQueryErrorResetBoundary = () => React14.useContext(QueryErrorResetBoundaryContext);
...
// src/utils/mcrApiClient.ts
var API_BASE_URL = import.meta.env.VITE_API_BASE_URL || "http://localhost:8000";
var USE_MOCKS = String(import.meta.env.VITE_USE_MCR_MOCKS || "false").toLowerCase() === "true";
var mcrApiClient = axios_default.create({
...
var toSlug = (value) => value.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "") || "unknown";
var normalizeRagLower = (value) => {
  const v = String(value || "").trim().toLowerCase();
  if (v === "green") return "green";
  if (v === "red") return "red";
  return "amber";
};
var normalizeRagTitle = (value) => {
  const rag = normalizeRagLower(value);
  if (rag === "green") return "Green";
  if (rag === "red") return "Red";
  return "Amber";
};
var toTriState = (value) => {
  const v = String(value || "").trim().toLowerCase();
  if (v === "green" || v === "met") return "Met";
  if (v === "red" || v === "not_met" || v === "not met") return "Not Met";
  return "Partially Met";
};
var toDeliveryStatus = (value) => {
  const v = String(value || "").trim().toLowerCase();
...
var toRecipient = (value) => {
  const v = String(value || "").trim().toLowerCase();
  if (v === "employer") return "Employer";
  if (v === "learner") return "Learner";
  return "QA";
};
var deriveListRating = (rawRating, score) => {
...
var deriveDetailRating = (listRating) => {
...
var metricToIndicatorKey = (metric) => {
...
var extractEvidenceUrls = (evidence) => {
  const urls = [];
...
var mapQaIndicators = (qaItems) => {
  return qaItems.map((item, idx) => {
    ...
    return {
      ...
      evidenceUrls: extractEvidenceUrls(asArray(item.evidence))
    };
  });
};
var timecodeToSeconds = (value) => {
...
var mapTranscriptEvidence = (qaItems) => {
  const byKey = /* @__PURE__ */ new Map();
  let order = 0;
  qaItems.forEach((qaItem) => {
    const metric = String(qaItem.metric || "General");
    asArray(qaItem.evidence).forEach((entry) => {
      ...
      byKey.set(key, {
        ...
        whyItMatters,
        metric,
        metricSet: /* @__PURE__ */ new Set([metric]),
        order: order++
      });
    });
  });
...
var buildMeetingSections = (qaItems, totalDurationMin, executiveSummary) => {
  ...
  return sections.map((section, index) => {
    ...
    const estimatedActual = isLast ? Math.max(0, totalDurationMin - allocated) : Math.max(0, Math.round(section.plannedMin / totalPlanned * totalDurationMin));
    ...
    const evidenceState = qaItem ? "Evidenced via QA indicators." : "Not explicitly evidenced in transcript metadata.";
    const notes = [
      qaComment,
      evidenceState,
...
var buildSafeguardingChecklist = (qaItems) => {
  const safeguardingQa = findQaItemByKeywords(qaItems, ["safeguarding"]);
  const status = toTriState(safeguardingQa?.rag);
...
var safeIsoDate = (value, fallback = (/* @__PURE__ */ new Date()).toISOString()) => {
  if (!value) return fallback;
  const d = new Date(value);
  return Number.isNaN(d.getTime()) ? fallback : d.toISOString();
};
var avgQaScoreFromIndicators = (qaIndicators) => {
  if (qaIndicators.length === 0) return 0;
  return qaIndicators.reduce((sum, item) => sum + item.score0to5, 0) / qaIndicators.length;
};
var mapCommunicationLog = (communications) => {
...
var mapDetailReview = (raw) => {
  const qaItems = asArray(raw.qa);
  ...
  const communicationLog = mapCommunicationLog(asArray(raw.communications));
  ...
  const areasForDevelopment = asArray(raw.priority_actions).map((item) => String(item.action || "").trim()).filter(Boolean);
  const summary = {
  ...
  const mapped = {
    id: String(raw.id),
    date: String(raw.date || raw.created_at || (/* @__PURE__ */ new Date()).toISOString()),
...
var getReviewById = async (id) => {
  const response = await mcrApiClient.get(`/api/mcr/reviews/${id}/`);
...
// src/pages/mcr-review-print/page.tsx
function McrReviewPrint() {
  const { id } = useParams();
  ...
  if (isLoading || !review) {
    return /* @__PURE__ */ React.createElement("div", { className: "flex items-center justify-center min-h-screen" }, /* @__PURE__ */ React.createElement("div", { className: "text-center" }, /* @__PURE__ */ React.createElement("div", { className: "w-12 h-12 border-4 border-teal-500 border-t-transparent rounded-full animate-spin mx-auto mb-4" }), /* @__PURE__ */ React.createElement("p", { className: "text-gray-600" }, "Loading review for print...")));
  }
  ...
  const communicationTargets = review.communicatedTo ?? { employer: false, learner: false, qa: false };
  ...
  const satisfactionScore = review.satisfaction?.score0to5 ?? review.satisfactionScore ?? 0;
  const overallQaScore = qaIndicators.length > 0 ? qaIndicators.reduce((sum, ind) => sum + (ind.score0to5 ?? 0), 0) / qaIndicators.length : 0;
  return /* @__PURE__ */ React.createElement("div", { className: "print-container bg-white p-8 max-w-[210mm] mx-auto" }, /* @__PURE__ */ React.createElement("style", null, `
        @media print {
      ...
      `), /* @__PURE__ */ React.createElement("div", { className: "border-b-2 border-gray-300 pb-6 mb-6" }, /* @__PURE__ */ React.createElement("div", { className: "flex items-center justify-between" }, /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("h1", { className: "text-3xl font-bold text-gray-900 mb-2" }, "Monthly Coach Review Report"), /* @__PURE__ */ React.createElement("p", { className: "text-sm text-gray-600" }, "Review ID: ", review.id)), /* @__PURE__ */ React.createElement("div", { className: "text-right" }, /* @__PURE__ */ React.createElement("p", { className: "text-sm text-gray-600" }, "Generated: ", format(/* @__PURE__ */ new Date(), "dd/MM/yyyy HH:mm")), /* @__PURE__ */ React.createElement("div", { className: `inline-block px-4 py-2 rounded-lg border-2 font-semibold text-sm mt-2 ${getRagColor(review.ragStatus)}` }, "RAG: ", String(review.ragStatus).toUpperCase())))), /* @__PURE__ */ React.createElement("section", { className: "mb-8" }, /* @__PURE__ */ React.createElement("h2", { className: "text-xl font-bold text-gray-900 mb-4 border-b border-gray-300 pb-2" }, "Review Overview"), /* @__PURE__ */ React.createElement("div", { className: "grid grid-cols-2 gap-4" }, /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("p", { className: "text-sm text-gray-600 mb-1" }, "Learner"), /* @__PURE__ */ React.createElement("p", { className: "font-semibold text-gray-900" }, review.learner)), /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("p", { className: "text-sm text-gray-600 mb-1" }, "Coach"), /* @__PURE__ */ React.createElement("p", { className: "font-semibold text-gray-900" }, review.coach)), /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("p", { className: "text-sm text-gray-600 mb-1" }, "Programme"), /* @__PURE__ */ React.createElement("p", { className: "font-semibold text-gray-900" }, review.programme)), /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("p", { className: "text-sm text-gray-600 mb-1" }, "Group"), /* @__PURE__ */ React.createElement("p", { className: "font-semibold text-gray-900" }, review.group)), /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("p", { className: "text-sm text-gray-600 mb-1" }, "Meeting Date"), /* @__PURE__ */ React.createElement("p", { className: "font-semibold text-gray-900" }, format(new Date(review.date), "dd MMMM yyyy"))), /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("p", { className: "text-sm text-gray-600 mb-1" }, "Total Duration"), /* @__PURE__ */ React.createElement("p", { className: "font-semibold text-gray-900" }, review.totalDurationMin, " minutes")), /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("p", { className: "text-sm text-gray-600 mb-1" }, "Overall QA Rating"), /* @__PURE__ */ React.createElement("p", { className: "font-semibold text-gray-900" }, review.qualitativeRating)), /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("p", { className: "text-sm text-gray-600 mb-1" }, "Satisfaction Score"), /* @__PURE__ */ React.createElement("p", { className: "font-semibold text-gray-900" }, Number(satisfactionScore).toFixed(1), "/5")))), /* @__PURE__ */ React.createElement("section", { className: "mb-8" }, /* @__PURE__ */ React.createElement("h2", { className: "text-xl font-bold text-gray-900 mb-4 border-b border-gray-300 pb-2" }, "Report Communication Coverage"), /* @__PURE__ */ React.createElement("div", { className: "grid grid-cols-3 gap-3" }, [
    { label: "Employer", value: communicationTargets.employer },
    { label: "Learner", value: communicationTargets.learner },
    { label: "Quality Assurance System", value: communicationTargets.qa }
  ].map((item) => /* @__PURE__ */ React.createElement("div", { key: item.label, className: "border border-gray-300 rounded-lg p-3" }, /* @__PURE__ */ React.createElement("p", { className: "text-xs text-gray-600 mb-1" }, item.label), /* @__PURE__ */ React.createElement("span", { className: `inline-block px-2 py-1 rounded text-xs font-semibold ${item.value ? "bg-green-100 text-green-800" : "bg-amber-100 text-amber-800"}` }, item.value ? "Communicated" : "Not Evidenced"))))), review.safeguardingFlagged && /* @__PURE__ */ React.createElement("section", { className: "mb-8 bg-red-50 border-2 border-red-500 rounded-lg p-4" }, /* @__PURE__ */ React.createElement("div", { className: "flex items-center gap-3" }, /* @__PURE__ */ React.createElement("div", { className: "w-10 h-10 bg-red-500 rounded-full flex items-center justify-center flex-shrink-0" }, /* @__PURE__ */ React.createElement("span", { className: "text-white font-bold text-xl" }, "!")), /* @__PURE__ */ React.createElement("div", null, /* @__PURE__ */ React.createElement("h3", { className: "font-bold text-red-900 mb-1" }, "Safeguarding Concern Flagged"), /* @__PURE__ */ React.createElement("p", { className: "text-sm text-red-800" }, "This review has been flagged for safeguarding concerns. Please review the safeguarding section carefully.")))), /* @__PURE__ */ React.createElement("section", { className: "mb-8 page-break" }, /* @__PURE__ */ React.createElement("h2", { className: "text-xl font-bold text-gray-900 mb-4 border-b border-gray-300 pb-2" }, "Meeting Structure & Timing"), /* @__PURE__ */ React.createElement("table", { className: "w-full border-collapse border border-gray-300 text-sm" }, /* @__PURE__ */ React.createElement("thead", { className: "bg-gray-100" }, /* @__PURE__ */ React.createElement("tr", null, /* @__PURE__ */ React.createElement("th", { className: "border border-gray-300 px-3 py-2 text-left" }, "Section"), /* @__PURE__ */ React.createElement("th", { className: "border border-gray-300 px-3 py-2 text-center" }, "Planned (min)"), /* @__PURE__ */ React.createElement("th", { className: "border border-gray-300 px-3 py-2 text-center" }, "Actual (min)"), /* @__PURE__ */ React.createElement("th", { className: "border border-gray-300 px-3 py-2 text-center" }, "Variance"), /* @__PURE__ */ React.createElement("th", { className: "border border-gray-300 px-3 py-2 text-left" }, "Notes"))), /* @__PURE__ */ React.createElement("tbody", null, meetingSections.map((section, index) => {
    const variance = section.actualMin - section.plannedMin;
    return /* @__PURE__ */ React.createElement("tr", { key: index, className: "hover:bg-gray-50" }, /* @__PURE__ */ React.createElement("td", { className: "border border-gray-300 px-3 py-2" }, section.sectionName), /* @__PURE__ */ React.createElement("td", { className: "border border-gray-300 px-3 py-2 text-center" }, section.plannedMin), /* @__PURE__ */ React.createElement("td", { className: "border border-gray-300 px-3 py-2 text-center font-semibold" }, section.actualMin), /* @__PURE__ */ React.createElement("td", { className: `border border-gray-300 px-3 py-2 text-center font-semibold ${variance > 0 ? "text-red-600" : variance < 0 ? "text-blue-600" : "text-green-600"}` }, variance > 0 ? "+" : "", variance), /* @__PURE__ */ React.createElement("td", { className: "border border-gray-300 px-3 py-2 text-xs" }, section.notes || "-"));
  }), /* @__PURE__ */ React.createElement("tr", { className: "bg-gray-100 font-semibold" }, /* @__PURE__ */ React.createElement("td", { className: "border border-gray-300 px-3 py-2" }, "Total"), /* @__PURE__ */ React.createElement("td", { className: "border border-gray-300 px-3 py-2 text-center" }, meetingSections.reduce((sum, s) => sum + s.plannedMin, 0)), /* @__PURE__ */ React.createElement("td", { className: "border border-gray-300 px-3 py-2 text-center" }, review.totalDurationMin), /* @__PURE__ */ React.createElement("td", { className: "border border-gray-300 px-3 py-2 text-center" }, review.totalDurationMin - meetingSections.reduce((sum, s) => sum + s.plannedMin, 0) > 0 ? "+" : "", review.totalDurationMin - meetingSections.reduce((sum, s) => sum + s.plannedMin, 0)), /* @__PURE__ */ React.createElement("td", { className: "border border-gray-300 px-3 py-2" }, "-"))))), /* @__PURE__ */ React.createElement("section", { className: "mb-8 page-break" }, /* @__PURE__ */ React.createElement("h2", { className: "text-xl font-bold text-gray-900 mb-4 border-b border-gray-300 pb-2" }, "QA Indicators"), /* @__PURE__ */ React.createElement("div", { className: "mb-4 p-3 bg-gray-100 rounded-lg" }, /* @__PURE__ */ React.createElement("p", { className: "text-sm text-gray-600" }, "Overall QA Score"), /* @__PURE__ */ React.createElement("p", { className: "text-2xl font-bold text-gray-900" }, overallQaScore.toFixed(1), "/5")), /* @__PURE__ */ React.createElement("div", { className: "space-y-4" }, qaIndicators.map((indicator, index) => /* @__PURE__ */ React.createElement("div", { key: index, className: "border border-gray-300 rounded-lg p-4" }, /* @__PURE__ */ React.createElement("div", { className: "flex items-start justify-between mb-2" }, /* @__PURE__ */ React.createElement("h3", { className: "font-semibold text-gray-900" }, indicator.indicatorName), /* @__PURE__ */ React.createElement("div", { className: "flex items-center gap-3" }, /* @__PURE__ */ React.createElement("span", { className: "text-lg font-bold text-gray-900" }, indicator.score0to5, "/5"), /* @__PURE__ */ React.createElement("span", { className: `px-3 py-1 rounded-full text-xs font-semibold ${getStatusColor(indicator.status)}` }, indicator.status))), indicator.comments && /* @__PURE__ */ React.createElement("p", { className: "text-sm text-gray-700 mt-2" }, indicator.comments), indicator.evidenceUrls && indicator.evidenceUrls.length > 0 && /* @__PURE__ */ React.createElement("div", { className: "mt-2" }, /* @__PURE__ */ React.createElement("p", { className: "text-xs text-gray-600 mb-1" }, "Evidence:"), /* @__PURE__ */ React.createElement("ul", { className: "text-xs text-gray-700 list-disc list-inside" }, indicator.evidenceUrls.map((url, i) => /* @__PURE__ */ React.createElement("li", { key: i }, url)))))))), /* @__PURE__ */ React.createElement("section", { className: "mb-8 page-break" }, /* @__PURE__ */ React.createElement("h2", { className: "text-xl font-bold text-gray-900 mb-4 border-b border-gray-300 pb-2" }, "Safeguarding & Wellbeing Checklist"), /* @__PURE__ */ React.createElement("table", { className: "w-full border-collapse border border-gray-300 text-sm" }, /* @__PURE__ */ React.createElement("thead", { className: "bg-gray-100" }, /* @__PURE__ */ React.createElement("tr", null, /* @__PURE__ */ React.createElement("th", { className: "border border-gray-300 px-3 py-2 text-left" }, "Checklist Item"), /* @__PURE__ */ React.createElement("th", { className: "border border-gray-300 px-3 py-2 text-center" }, "Status"), /* @__PURE__ */ React.createElement("th", { className: "border border-gray-300 px-3 py-2 text-left" }, "Notes"))), /* @__PURE__ */ React.createElement("tbody", null, safeguardingChecklist.map((item, index) => /* @__PURE__ */ React.createElement("tr", { key: index, className: "hover:bg-gray-50" }, /* @__PURE__ */ React.createElement("td", { className: "border border-gray-300 px-3 py-2" }, item.label), /* @__PURE__ */ React.createElement("td", { className: "border border-gray-300 px-3 py-2 text-center" }, /* @__PURE__ */ React.createElement("span", { className: `inline-block px-3 py-1 rounded-full text-xs font-semibold ${getStatusColor(item.status)}` }, item.status)), /* @__PURE__ */ React.createElement("td", { className: "border border-gray-300 px-3 py-2 text-xs" }, item.notes || "-")))))), /* @__PURE__ */ React.createElement("section", { className: "mb-8 page-break" }, /* @__PURE__ */ React.createElement("h2", { className: "text-xl font-bold text-gray-900 mb-4 border-b border-gray-300 pb-2" }, "Transcript Evidence"), transcriptEvidence.length > 0 ? /* @__PURE__ */ React.createElement("table", { className: "w-full border-collapse border border-gray-300 text-sm" }, /* @__PURE__ */ React.createElement("thead", { className: "bg-gray-100" }, /* @__PURE__ */ React.createElement("tr", null, /* @__PURE__ */ React.createElement("th", { className: "border border-gray-300 px-3 py-2 text-left" }, "Timestamp"), /* @__PURE__ */ React.createElement("th", { className: "border border-gray-300 px-3 py-2 text-left" }, "Speaker"), /* @__PURE__ */ React.createElement("th", { className: "border border-gray-300 px-3 py-2 text-left" }, "Quote"), /* @__PURE__ */ React.createElement("th", { className: "border border-gray-300 px-3 py-2 text-left" }, "QA Metric"))), /* @__PURE__ */ React.createElement("tbody", null, transcriptEvidence.map((item, index) => /* @__PURE__ */ React.createElement("tr", { key: index, className: "hover:bg-gray-50 align-top" }, /* @__PURE__ */ React.createElement("td", { className: "border border-gray-300 px-3 py-2 font-mono text-xs" }, item.timestamp), /* @__PURE__ */ React.createElement("td", { className: "border border-gray-300 px-3 py-2 text-xs font-semibold" }, item.speaker), /* @__PURE__ */ React.createElement("td", { className: "border border-gray-300 px-3 py-2 text-xs" }, item.text), /* @__PURE__ */ React.createElement("td", { className: "border border-gray-300 px-3 py-2 text-xs" }, item.metric || "-"))))) : /* @__PURE__ */ React.createElement("p", { className: "text-sm text-gray-600" }, "No transcript evidence available for this review.")), /* @__PURE__ */ React.createElement("section", { className: "mb-8 page-break" }, /* @__PURE__ */ React.createElement("h2", { className: "text-xl font-bold text-gray-900 mb-4 border-b border-gray-300 pb-2" }, "Overall Summary"), /* @__PURE__ */ React.createElement("div", { className: "mb-6" }, /* @__PURE__ */ React.createElement("h3", { className: "font-semibold text-gray-900 mb-2" }, "Executive Summary"), /* @__PURE__ */ React.createElement("p", { className: "text-sm text-gray-700 leading-relaxed" }, summary.executiveSummary || "-")), /* @__PURE__ */ React.createElement("div", { className: "mb-6" }, /* @__PURE__ */ React.createElement("h3", { className: "font-semibold text-gray-900 mb-2" }, "Strengths"), summary.strengths.length > 0 ? /* @__PURE__ */ React.createElement("ul", { className: "list-disc list-inside space-y-1" }, summary.strengths.map((strength, index) => /* @__PURE__ */ React.createElement("li", { key: index, className: "text-sm text-gray-700" }, strength))) : /* @__PURE__ */ React.createElement("p", { className: "text-sm text-gray-600" }, "No strengths captured.")), /* @__PURE__ */ React.createElement("div", { className: "mb-6" }, /* @__PURE__ */ React.createElement("h3", { className: "font-semibold text-gray-900 mb-2" }, "Areas for Development"), summary.areasForDevelopment.length > 0 ? /* @__PURE__ */ React.createElement("ul", { className: "list-disc list-inside space-y-1" }, summary.areasForDevelopment.map((area, index) => /* @__PURE__ */ React.createElement("li", { key: index, className: "text-sm text-gray-700" }, area))) : /* @__PURE__ */ React.createElement("p", { className: "text-sm text-gray-600" }, "No development areas captured.")), /* @__PURE__ */ React.createElement("div", { className: "mb-6" }, /* @__PURE__ */ React.createElement("h3", { className: "font-semibold text-gray-900 mb-2" }, "Professional Judgement"), /* @__PURE__ */ React.createElement("p", { className: "text-sm text-gray-700 leading-relaxed" }, summary.professionalJudgement || "-"))), /* @__PURE__ */ React.createElement("section", { className: "mb-8" }, /* @__PURE__ */ React.createElement("h2", { className: "text-xl font-bold text-gray-900 mb-4 border-b border-gray-300 pb-2" }, "Attachments"), review.attachments && review.attachments.length > 0 ? /* @__PURE__ */ React.createElement("table", { className: "w-full border-collapse border border-gray-300 text-sm" }, /* @__PURE__ */ React.createElement("thead", { className: "bg-gray-100" }, /* @__PURE__ */ React.createElement("tr", null, /* @__PURE__ */ React.createElement("th", { className: "border border-gray-300 px-3 py-2 text-left" }, "File Name"), /* @__PURE__ */ React.createElement("th", { className: "border border-gray-300 px-3 py-2 text-left" }, "Type"), /* @__PURE__ */ React.createElement("th", { className: "border border-gray-300 px-3 py-2 text-left" }, "URL"))), /* @__PURE__ */ React.createElement("tbody", null, review.attachments.map((attachment, index) => /* @__PURE__ */ React.createElement("tr", { key: index, className: "hover:bg-gray-50" }, /* @__PURE__ */ React.createElement("td", { className: "border border-gray-300 px-3 py-2" }, attachment.name), /* @__PURE__ */ React.createElement("td", { className: "border border-gray-300 px-3 py-2" }, attachment.type), /* @__PURE__ */ React.createElement("td", { className: "border border-gray-300 px-3 py-2 text-xs break-all" }, attachment.url))))) : /* @__PURE__ */ React.createElement("p", { className: "text-sm text-gray-600" }, "No attachments available.")), /* @__PURE__ */ React.createElement("section", { className: "mb-8 page-break" }, /* @__PURE__ */ React.createElement("h2", { className: "text-xl font-bold text-gray-900 mb-4 border-b border-gray-300 pb-2" }, "Communication Log"), review.communicationLog && review.communicationLog.length > 0 ? /* @__PURE__ */ React.createElement("table", { className: "w-full border-collapse border border-gray-300 text-sm" }, /* @__PURE__ */ React.createElement("thead", { className: "bg-gray-100" }, /* @__PURE__ */ React.createElement("tr", null, /* @__PURE__ */ React.createElement("th", { className: "border border-gray-300 px-3 py-2 text-left" }, "Date & Time"), /* @__PURE__ */ React.createElement("th", { className: "border border-gray-300 px-3 py-2 text-left" }, "Recipient"), /* @__PURE__ */ React.createElement("th", { className: "border border-gray-300 px-3 py-2 text-left" }, "Sent By"), /* @__PURE__ */ React.createElement("th", { className: "border border-gray-300 px-3 py-2 text-center" }, "Status"), /* @__PURE__ */ React.createElement("th", { className: "border border-gray-300 px-3 py-2 text-left" }, "Notes"))), /* @__PURE__ */ React.createElement("tbody", null, review.communicationLog.map((log, index) => /* @__PURE__ */ React.createElement("tr", { key: index, className: "hover:bg-gray-50" }, /* @__PURE__ */ React.createElement("td", { className: "border border-gray-300 px-3 py-2" }, format(new Date(log.sentAt), "dd/MM/yyyy HH:mm")), /* @__PURE__ */ React.createElement("td", { className: "border border-gray-300 px-3 py-2" }, log.recipientType), /* @__PURE__ */ React.createElement("td", { className: "border border-gray-300 px-3 py-2" }, log.sentBy), /* @__PURE__ */ React.createElement("td", { className: "border border-gray-300 px-3 py-2 text-center" }, /* @__PURE__ */ React.createElement("span", { className: `inline-block px-3 py-1 rounded-full text-xs font-semibold ${log.status === "Sent" ? "bg-green-100 text-green-800" : log.status === "Pending" ? "bg-amber-100 text-amber-800" : "bg-red-100 text-red-800"}` }, log.status)), /* @__PURE__ */ React.createElement("td", { className: "border border-gray-300 px-3 py-2 text-xs" }, log.notes || "-"))))) : /* @__PURE__ */ React.createElement("p", { className: "text-sm text-gray-600" }, "No communication log entries available.")), /* @__PURE__ */ React.createElement("div", { className: "border-t-2 border-gray-300 pt-4 mt-8 text-center text-xs text-gray-600" }, /* @__PURE__ */ React.createElement("p", null, "Monthly Coach Review Report - Generated on ", format(/* @__PURE__ */ new Date(), "dd MMMM yyyy HH:mm")), /* @__PURE__ */ React.createElement("p", { className: "mt-1" }, "This is a confidential document. Please handle with care.")), /* @__PURE__ */ React.createElement("div", { className: "no-print fixed bottom-6 right-6" }, /* @__PURE__ */ React.createElement(
    "button",
    {
      onClick: () => window.print(),
      className: "px-6 py-3 bg-teal-500 text-white font-semibold rounded-lg shadow-lg hover:bg-teal-600 transition-colors cursor-pointer whitespace-nowrap flex items-center gap-2"
    },
    /* @__PURE__ */ React.createElement("i", { className: "ri-printer-line text-xl" }),
    "Print / Save as PDF"
  )));
}
export {
  McrReviewPrint as default
};
